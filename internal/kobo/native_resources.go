...
// nativeResources is a real /v1/initialization map, captured from a running server
// on 2026-08-24 rather than typed out. It is what a device gets when there is no map
// on disk and the store will not hand one over.
//
// One map of any rather than anything narrower, because that is the shape the
// response needs and two of the values Kobo sends are objects. The keys this server
// serves are replaced per request by resourceOverrides; everything here is left as
// Kobo has it.
//
// Two things were changed on the way in. The four keys the capturing instance had
// pointed at itself are back at Kobo's own values, so nobody's address ships here.
// And three hosts were dropped because they no longer resolve: social_host,
// social_authorization_host and discovery_host, all *.kobobooks.com. Every key of
