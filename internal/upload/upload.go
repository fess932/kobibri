...
type Item struct {
	...
	Converted bool
	// Converting says it is still queued. Not the negation of Converted: a file
	// uploaded as .kepub.epub is served untouched and never queues, so it is
	// neither, and showing "converting" for it never stopped being wrong.
	Converting bool
...
func (u *Store) List(ctx context.Context) ([]Item, error) {
	rows, err := u.store.Reader().QueryContext(ctx, `
		SELECT sb.id, COALESCE(sb.book_id, ''), sb.title, sb.authors_json,
		       ...
		       COALESCE((SELECT `+store.AwaitingConversionSQL("b")+` FROM books b
		                  WHERE b.id = sb.book_id), 0),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&it.SourceBookID, &it.BookID, &it.Title, &authorsJSON,
			...
			&it.Converted, &it.Converting, &it.Syncable); err != nil {
			return nil, err
		}
		var authors []string
